...
export interface RendererRichOptions {
   ...
   */
  renderMarkdown?: (this: CodelexTransformerContextCommon, markdown: string) => ElementContent[]
   ...
   * By default it pass-through the markdown.
   */
  renderMarkdownInline?: (this: CodelexTransformerContextCommon, markdown: string, context: string) => ElementContent[]
...
export function rendererRich(options: RendererRichOptions = {}): TwoslashRenderer {
  ...
  function highlightPopupContent(
    this: CodelexTransformerContextCommon,
  ...
  return {
    ...
    nodeCompletion(query, node) {
      if (node.type !== 'text')
        throw new CodelexTwoslashError(`Renderer hook nodeCompletion only works on text nodes, got ${node.type}`)
