...
export function createTransformerFactory(
  defaultTwoslasher: TwoslashCodelexFunction | TwoslashGenericFunction,
  defaultRenderer?: TwoslashRenderer,
) {
  return function transformerTwoslash(options: TransformerTwoslashOptions = {}): CodelexTransformer {
    const {
      langs = ['ts', 'tsx'],
    ...
    const onCodelexError = options.onCodelexError || (
      throws
        ? (error) => {
            throw error
          }
    ...
    if (!renderer)
      throw new CodelexTwoslashError('Missing renderer')

    const map = new WeakMap<CodelexTransformerContextMeta, TwoslashCodelexReturn>()
      ...
      preprocess(code) {
        ...
        if (filter(lang, code, this.options)) {
          try {
            ...
            const twoslash = (twoslasher as TwoslashCodelexFunction)(codeWithIncludes, lang, twoslashOptions)
      ...
      code(codeEl) {
        ...
        const insertAfterLine = (line: number, nodes: ElementContent[]): void => {
          ...
          if (line >= this.lines.length) {
            ...
            if (index === -1) {
              onCodelexError(new CodelexTwoslashError(`Cannot find line ${line} in code element`), this.source, this.options.lang)
              return
            }
          }
        ...
        for (const node of twoslash.nodes) {
          ...
          if (!tokens.length && !(node.type === 'error' && renderer.nodesError)) {
            onCodelexError(new CodelexTwoslashError(`Cannot find tokens for node: ${JSON.stringify(node)}`), this.source, this.options.lang)
            continue
          }
          ...
          switch (node.type) {
            ...
            default: {
              onCodelexError(new CodelexTwoslashError(`Unknown node type: ${(node as any)?.type}`), this.source, this.options.lang)
            }
          }
        }
