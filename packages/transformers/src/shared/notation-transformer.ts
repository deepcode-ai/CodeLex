...
export interface MatchAlgorithmOptions {
   ...
   *
   * @see https://codelex.style/packages/transformers#matching-algorithm
   * @default 'v3'
   */
...
export function createCommentNotationTransformer(
  ...
  regex: RegExp,
  onMatch: (
    this: CodelexTransformerContext,
    match: string[],
    line: Element,
    commentNode: Element,
    lines: Element[],
  ...
  return {
    ...
    code(code) {
      ...
      const data = code.data as {
        _codelex_notation?: ParsedComments
      }

      data._codelex_notation ??= parseComments(lines, ['jsx', 'tsx'].includes(this.options.lang), matchAlgorithm)
      const parsed = data._codelex_notation
