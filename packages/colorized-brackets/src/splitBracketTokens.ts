import type { ThemedToken } from 'codelex'
...
export default function splitBracketTokens(
  ...
  const explanationsWithStartEnd = (explanations ?? []).map(
    (explanation, i) => {
      ...
      // with codelex option mergeWhitespaces (default true), the leading/trailing whitespaces of the token and explanations do not necessarily match
