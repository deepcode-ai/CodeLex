import type { CodeOptionsSingleTheme, CodeOptionsThemes, ThemedToken } from 'codelex'
...
export default function colorizeBracketTokens(
  ...
  codelexOptions: CodeOptionsThemes,
  lang: string,
): void {
  ...
  for (const token of tokens) {
    ...
    if (openers.has(token.content.trim())) {
      ...
      if (opener) {
        ...
        assignColorToToken(token, resolvedConfig.themes, codelexOptions, -1)
      }
    }
  }
...
function assignColorToToken(
  ...
  if (isSingleTheme(codelexOptions)) {
    const themeName
      = typeof codelexOptions.theme === 'string'
        ? codelexOptions.theme
        : codelexOptions.theme.name
    token.color = getColor(themes, themeName, level)
  }
  else {
    const { defaultColor = 'light', cssVariablePrefix = '--codelex-' }
      = codelexOptions
    ...
    for (const [colorName, theme] of Object.entries(codelexOptions.themes)) {
...
function isSingleTheme(
  codelexOptions: CodeOptionsThemes,
): codelexOptions is CodeOptionsSingleTheme {
  return 'theme' in codelexOptions
}
