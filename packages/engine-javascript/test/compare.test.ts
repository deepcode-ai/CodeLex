...
import type { LanguageRegistration, RegexEngine, ThemeRegistration } from '../../codelex/src/core'
import type { Execution } from './types'

import { hash as createHash } from 'ohash'
import { describe, expect, it } from 'vitest'
import { createOnigurumaEngine, loadWasm } from '../../engine-oniguruma/src'
import { createHighlighterCore } from '../../codelex/src/core'
...
describe('cases', async () => {
  ...
  for (const c of resolved) {
    ...
    run(c.c.name, async () => {
      ...
      using codelex1 = await createHighlighterCore({
        langs: c.lang,
        themes: [c.theme],
        engine: engineWasm,
      })
      using codelex2 = await createHighlighterCore({
        langs: c.lang,
        themes: [c.theme],
        engine: engineJs,
      })
      ...
      for (const code of c.c.cases) {
        compare.push([
          codelex1.codeToTokensBase(code, { lang, theme }),
          codelex2.codeToTokensBase(code, { lang, theme }),
        ])
      }
