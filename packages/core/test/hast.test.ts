...
import { codeToHtml, createHighlighter } from 'codelex'
...
describe('should', () => {
  it('works', async () => {
    ...
    const hast = codelex.codeToHast('console.log\nfoo.bar', { lang: 'js', theme: 'vitesse-light' })

    expect(toHtml(hast))
      .toMatchInlineSnapshot(`
        "<pre class="codelex vitesse-light" style="background-color:#ffffff;color:#393a34" tabindex="0"><code><span class="line"><span style="color:#B07D48">console</span><span style="color:#999999">.</span><span style="color:#B07D48">log</span></span>
...
it('render whitespace', async () => {
  ...
  expect(code)
    .toMatchInlineSnapshot(`
      "<pre class="codelex vitesse-light" style="background-color:#ffffff;color:#393a34" tabindex="0"><code><span class="line"><span style="color:#59873A">  space</span><span style="color:#999999">()</span></span>
...
describe('merge same style', () => {
  it('merges adjacent tokens with same style', async () => {
    ...
    const code = 'name: CI'
    const html = await codelex.codeToHtml(code, {
      lang: 'yaml',
      theme: 'min-light',
      mergeSameStyleTokens: true,
    })

    expect(html).toMatchInlineSnapshot(`"<pre class="codelex min-light" style="background-color:#ffffff;color:#24292eff" tabindex="0"><code><span class="line"><span style="color:#D32F2F">name:</span><span style="color:#22863A"> CI</span></span></code></pre>"`)
  })
  ...
  it('does not merge tokens with decorations', async () => {
    ...
    expect(html).toMatchInlineSnapshot(`"<pre class="codelex min-light" style="background-color:#ffffff;color:#24292eff" tabindex="0"><code><span class="line"><span style="color:#D32F2F" class="highlighted-word">name</span><span style="color:#D32F2F">:</span><span style="color:#22863A"> CI</span></span></code></pre>"`)
  })
})
