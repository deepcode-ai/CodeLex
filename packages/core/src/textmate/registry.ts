...
export class Registry extends TextMateRegistry {
    ...
    if (this._alias[name]) {
      ...
      while (this._alias[name]) {
        ...
        if (resolved.has(name))
          throw new CodelexError(`Circular alias \`${Array.from(resolved).join(' -> ')} -> ${name}\``)
        resolved.add(name)
      }
    }
  ...
  public loadLanguages(langs: LanguageRegistration[]): void {
    ...
    if (missingLangs.length) {
      ...
      throw new CodelexError(`Missing languages ${missingLangs.map(([name]) => `\`${name}\``).join(', ')}, required by ${dependents.map(([name]) => `\`${name}\``).join(', ')}`)
    }
