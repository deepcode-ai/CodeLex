  ...
  async function createHighlighter(
    ...
    function resolveLang(lang: LanguageInput | BundledLangs | SpecialLanguage): LanguageInput {
      if (typeof lang === 'string') {
        ...
        if (!bundle)
          throw new CodelexError(`Language \`${lang}\` is not included in this bundle. You may want to load it from external source.`)
        return bundle
      }
    ...
    function resolveTheme(theme: ThemeInput | BundledThemes | SpecialTheme): ThemeInput | SpecialTheme {
      ...
      if (typeof theme === 'string') {
        ...
        if (!bundle)
          throw new CodelexError(`Theme \`${theme}\` is not included in this bundle. You may want to load it from external source.`)
        return bundle
      }
      return theme
    }
...
export function makeSingletonHighlighter<L extends string, T extends string>(
  ...
  let _codelex: ReturnType<typeof createHighlighter>
...
export function createSingletonShorthands<L extends string, T extends string>(
  ...
  async function get(code: string, options: CodeToTokensOptions<L, T> | CodeToHastOptions<L, T>): Promise<HighlighterGeneric<L, T>> {
    ...
    const langs = await config?.guessEmbeddedLanguages?.(code, options.lang, codelex) as L[]
    if (langs) {
      await codelex.loadLanguage(...langs)
    }
    return codelex
  }
  ...
  return {
    ...
    async codeToHtml(code, options) {
      const codelex = await get(code, options)
      return codelex.codeToHtml(code, options)
    },

    async codeToHast(code, options) {
      const codelex = await get(code, options)
      return codelex.codeToHast(code, options)
    },

    async codeToTokens(code, options) {
      const codelex = await get(code, options)
      return codelex.codeToTokens(code, options)
    },

    async codeToTokensBase(code, options) {
      const codelex = await get(code, options)
      return codelex.codeToTokensBase(code, options)
    },

    async codeToTokensWithThemes(code, options) {
      const codelex = await get(code, options)
      return codelex.codeToTokensWithThemes(code, options)
    },
    ...
    async getLastGrammarState(code, options) {
      ...
      return codelex.getLastGrammarState(code, options)
    },
  }
}
