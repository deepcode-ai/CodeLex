 ...
 *
 * Synchronous version of `createCodelexInternal`, which requires to provide the engine and all themes and languages upfront.
 */
export function createCodelexInternalSync(options: HighlighterCoreOptions<true>): CodelexInternal {
  instancesCount += 1
  if (options.warnings !== false && instancesCount >= 10 && instancesCount % 10 === 0)
    console.warn(`[Codelex] ${instancesCount} instances have been created. Codelex is supposed to be used as a singleton, consider refactoring your code to cache your highlighter instance; Or call \`highlighter.dispose()\` to release unused instances.`)

  let isDisposed = false

  if (!options.engine)
    throw new CodelexError('`engine` option is required for synchronous mode')
  ...
  function getLanguage(name: string | LanguageRegistration): Grammar {
    ...
    if (!_lang)
      throw new CodelexError(`Language \`${name}\` not found, you may need to load it first`)
    return _lang
  }
  ...
  function getTheme(name: string | ThemeRegistrationAny): ThemeRegistrationResolved {
    ...
    if (!_theme)
      throw new CodelexError(`Theme \`${name}\` not found, you may need to load it first`)
    return _theme
  }
  ...
  function ensureNotDisposed(): void {
    if (isDisposed)
      throw new CodelexError('Codelex instance has been disposed')
  }
