...
export function codeToTokensBase(
  ...
  if (options.grammarState) {
    if (options.grammarState.lang !== _grammar.name) {
      throw new CodelexError(`Grammar state language "${options.grammarState.lang}" does not match highlight language "${_grammar.name}"`)
    }
    if (!options.grammarState.themes.includes(theme.name)) {
      throw new CodelexError(`Grammar state themes "${options.grammarState.themes}" do not contain highlight theme "${theme.name}"`)
    }
  }
...
export function getLastGrammarState(...args: any[]): GrammarState | undefined {
  ...
  const [internal, code, options = {}] = args as [CodelexInternal, string, CodeToTokensBaseOptions]
  const {
    lang = 'text',
    theme: themeName = internal.getLoadedThemes()[0],
  } = options
  ...
  if (isPlainLang(lang) || isNoneTheme(themeName))
    throw new CodelexError('Plain language does not have grammar state')
  if (lang === 'ansi')
    throw new CodelexError('ANSI language does not have grammar state')
