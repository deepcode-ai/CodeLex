...
function throwLastOnigError(onigBinding: IOnigBinding): void {
  throw new CodelexError(onigBinding.UTF8ToString(onigBinding.getLastOnigError()))
}
...
export class OnigString implements IOnigString {
  ...
  constructor(str: string) {
    if (!onigBinding)
      throw new CodelexError('Must invoke loadWasm first.')
  ...
  async function _load(): Promise<void> {
    onigBinding = await createOnigasm(async (info) => {
      ...
      if (isInstantiatorOptionsObject(instance)) {
        ...
        // import("codelex/onig.wasm") returns `{ default: WebAssembly.Module }` on cloudflare workers
