...
`
  ...
  it('multiple themes', async () => {
    ...
    const code = await codeToHtml('import * as Codelex from "codelex"', {
      lang: 'js',
      themes,
      cssVariablePrefix: '--s-',
    })
...
[data-theme="${theme}"] .codelex,
[data-theme="${theme}"] .codelex span {
...
`
  ...
  it('should support font style', async () => {
    const input = 'import * as Codelex from \'codelex\';\n'
    ...
    expect(code1)
      .toContain('--codelex-dark-font-style:inherit')
    ...
    expect(code2)
      .toContain('--codelex-light-font-style:italic')
  })
...
describe('errors', () => {
  it('throws on empty theme', async () => {
    await expect(() => codeToHtml('console.log("hello")', {
      ...
      .rejects
      .toThrowErrorMatchingInlineSnapshot(`[CodelexError: \`themes\` option must not be empty]`)
  })
  ...
  it('throws on missing default color', async () => {
    await expect(() => codeToHtml('console.log("hello")', {
      ...
      .rejects
      .toThrowErrorMatchingInlineSnapshot(`[CodelexError: \`themes\` option must contain the defaultColor key \`light\`]`)
    ...
    await expect(() => codeToHtml('console.log("hello")', {
      ...
      .rejects
      .toThrowErrorMatchingInlineSnapshot(`[CodelexError: \`themes\` option must contain the defaultColor key \`dark\`]`)
  })
  ...
  it('not throws when `defaultColor` set to false', async () => {
    ...
    expect(code).toMatchInlineSnapshot(`"<pre class="codelex codelex-themes nord" style="--codelex-dark:#d8dee9ff;--codelex-dark-bg:#2e3440ff" tabindex="0"><code><span class="line"><span style="--codelex-dark:#D8DEE9">console</span><span style="--codelex-dark:#ECEFF4">.</span><span style="--codelex-dark:#88C0D0">log</span><span style="--codelex-dark:#D8DEE9FF">(</span><span style="--codelex-dark:#ECEFF4">"</span><span style="--codelex-dark:#A3BE8C">hello</span><span style="--codelex-dark:#ECEFF4">"</span><span style="--codelex-dark:#D8DEE9FF">)</span></span></code></pre>"`)
  })
})
