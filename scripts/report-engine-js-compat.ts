...
import type { BundledLanguage, BundledTheme, HighlighterGeneric } from 'codelex'
import fs from 'node:fs/promises'
import process from 'node:process'
import c from 'ansis'
import { diffCleanupSemantic, diffMain } from 'diff-match-patch-es'
import { format } from 'prettier'
import { bundledLanguages, createHighlighter, createJavaScriptRegexEngine } from 'codelex'
...
async function run() {
  ...
  for (const lang of Object.keys(bundledLanguages)) {
    ...
    const grammars = codelexWasm.getLoadedLanguages().map(l => codelexWasm.getLanguage(l))
    ...
    const highlightA = serializeTokens(codelexWasm, sample, lang)
    let highlightB: { tokens: string, html: string } | undefined
    let highlightDiff: Diff[] = []

    try {
      codelex = await createHighlighter({
        langs: [lang],
        themes: ['vitesse-dark'],
        engine,
      })

      highlightB = serializeTokens(codelex, sample, lang)
    }
    catch (e: any) {
...
function serializeTokens(codelex: HighlighterGeneric<BundledLanguage, BundledTheme>, sample: string, lang: string) {
  const tokens = codelex
