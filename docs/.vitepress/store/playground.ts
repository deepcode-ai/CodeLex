...
export const usePlayground = defineStore('playground', () => {
  const lang = useLocalStorage('codelex-playground-lang', 'typescript')
  const theme = useLocalStorage('codelex-playground-theme', 'vitesse-dark')
  ...
  const input = useLocalStorage('codelex-playground-input', '')
  ...
  ;(async () => {
    ...
    const { bundledLanguagesInfo: bundleFull } = await import('codelex/bundle/full')
    const { bundledLanguagesInfo: bundleWeb } = await import('codelex/bundle/web')
    const { bundledThemesInfo } = await import('codelex/themes')
      ...
      function run(): void {
        output.value = highlighter.codeToHtml(input.value, {
          ...
          transformers: [
            {
              preprocess(code) {
                // Workaround for https://github.com/deepcode-ai/codelex/issues/608
