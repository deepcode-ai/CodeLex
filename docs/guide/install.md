...
The quickest way to get started with `codelex` is to use the provided shorthand functions. These will load the necessary themes and languages on demand, and automatically cache them in memory.
...
import { codeToTokens } from 'codelex'
...
import { codeToHast } from 'codelex'
...
Since Codelex v1.0, it requires all themes and languages to be loaded explicitly.
...
import { bundledLanguages, bundledThemes, createHighlighter } from 'codelex'
...
`codelex` is published as ESM-only to reduce the package size. It's still possible to use it in CJS, as Node.js supports importing ESM modules dynamically in CJS.
...
// CJS
async function main() {
  const { createHighlighter } = await import('codelex')
...
To use `codelex` in the browser via CDN, you can use [esm.run](https://esm.run) or [esm.sh](https://esm.sh).
...
<body>
  ...
  <script type="module">
    ...
    import { codeToHtml } from 'https://esm.sh/codelex@3.0.0'
    // or
    // import { codeToHtml } from 'https://esm.run/codelex@3.0.0'

    const foo = document.getElementById('foo')
    foo.innerHTML = await codeToHtml('console.log("Hi, Codelex on CDN :)")', {
      lang: 'js',
      theme: 'rose-pine'
    })
  </script>
...
await loadWasm(import('codelex/onig.wasm'))
...
export default {
  async fetch() {
    ...
    return new Response(highlighter.codeToHtml('console.log(\'codelex\');', {
      theme: 'nord',
      lang: 'js'
    }))
  },
