...
Codelex does not provide an official integration for [Next.js](https://nextjs.org), but it is rather straightforward to use Codelex in Next.js applications.

::: info
Using Codelex on Edge Runtime might cause unintended problems, Codelex relies on lazy imports to load languages and themes.
...
```ts
import type { JSX } from 'react'
import type { BundledLanguage } from 'codelex/bundle/web'
import { toJsxRuntime } from 'hast-util-to-jsx-runtime'
import { Fragment } from 'react'
import { jsx, jsxs } from 'react/jsx-runtime'
import { codeToHast } from 'codelex/bundle/web'
...
::: info
The above example uses the `codelex/bundle/web` bundle. You can change it to [Fine-grained Bundle](/guide/bundles#fine-grained-bundle) to fully control the bundled languages/themes.
:::

### Performance

Codelex lazy loads the requested languages and themes, the Next.js bundler can handle lazy imports automatically.
Importing `codelex` or its web bundle is efficient enough for most Next.js applications, Fine-grained Bundle won't significantly impact the bundle size.
